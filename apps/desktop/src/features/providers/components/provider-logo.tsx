...
const PROVIDER_LOGOS: Record<string, { src: string; label: string }> = {
  claude: { src: claudeLogo, label: "Claude" },
};
...
export function ProviderLogo({
  ...
  const logo = PROVIDER_LOGOS[normalized];
  if (!logo) return null;
  return (
    <img
      src={logo.src}
...
function OpenAiLogo({ className }: { className?: string }) {
  return (
    <svg
      ...
      viewBox="0 0 24 24"
      aria-label="OpenAI"
      className={cn("size-3 shrink-0 text-current", className)}
    >
...
export function ProviderModelLabel({
  ...
  return (
    <span className={cn("inline-flex min-w-0 items-center gap-1", className)}>
      <ProviderLogo provider={provider} className={logoClassName} />
